...
private:
    ...
    // ADD MORE DATA MEMBERS HERE, AS NECESSARY
		mutable size_t numItems_;
		mutable size_t numTombstones_;
		double resizeAlpha_;
};
...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), table_(CAPACITIES[0], nullptr), mIndex_(0),
			 		resizeAlpha_(resizeAlpha), totalProbes_(0), numItems_(0), numTombstones_(0)
{
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if(npos == loc){
		...
		// if key occupies the space, update the value of the key
		else {
...
{
		// probe the key. if location is already nullptr, no deletion required
    HASH_INDEX_T loc = this->probe(key);
    if((npos == loc) || nullptr == table_[loc] || table_[loc]->deleted == true){
        return;
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
	...
	for (size_t i = 0; i < prev.size(); i++) {
		...
		// free the memory of the prev vector
		delete prev[i];
	}
}
